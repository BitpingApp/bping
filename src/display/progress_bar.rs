use crate::{app_config, custom_validators};
...
pub fn get_progress_bar_text(endpoint: &str, regions: &Vec<String>) -> String {
  ...
  let regions_display = match regions.len() {
      ...
      _ => regions.iter().map(|region| {
        let mut region_msg = region.to_owned();
  
        if config.show_emojis == true {
          if let Some(code) = custom_validators::get_emoji_safe_region_code(region) {
            if let Some(emoji) = emojis::get_emoji_for_country_code(code) {
              region_msg = format!("{}", console::Emoji(&format!("{} ", emoji), region))
            }
          }
        }
        region_msg
